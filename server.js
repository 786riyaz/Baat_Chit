...
} = require("./server/socket/handlers/receipts");
  ...
  return (req, res, next) => {
    ...
    if (limited) {
      return res.status(429).json({
        ...
        message:
          message ||
          `Too many requests. Please try again in ${Math.ceil(retryAfterMs / 60000)} minute(s).`,
      });
    }
    next();
  };
...
app.get("/api/admin-contact", authenticateToken, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Unable to load admin contact" });
  }
});
// ---------------- PROFILE MANAGEMENT ----------------
app.put("/api/users/me", authenticateToken, async (req, res) => {
  try {
    ...
    const { name, phone, bio, lastSeenPrivacy, currentPassword, newPassword } =
      req.body;
    ...
    if (phone !== undefined) {
      ...
      if (!trimmedPhone) {
        return res
          .status(400)
          .json({ success: false, message: "Phone cannot be empty" });
      }
    ...
    if (lastSeenPrivacy !== undefined) {
      if (!["everyone", "nobody"].includes(lastSeenPrivacy)) {
        return res
          ...
          .json({
            success: false,
            message: "lastSeenPrivacy must be 'everyone' or 'nobody'",
          });
      }
  ...
  async (req, res) => {
    try {
      if (!req.file) {
        return res
          .status(400)
          .json({ success: false, message: "Profile photo file is required" });
      }
      const user = await User.findById(req.user.userId);
      if (!user) {
        return res
...
app.get("/api/users/exists", authenticateToken, async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }).select(
      "_id name email isOnline lastSeen lastSeenPrivacy",
    );
    if (!user) {
    ...
    const [activeRooms, archivedRooms] = await Promise.all([
      ...
      ArchivedChat.aggregate([
        { $match: { chatType: "personal", roomId: roomPattern } },
        {
          $group: {
            _id: "$roomId",
            lastMessageAt: { $max: "$originalCreatedAt" },
          },
        },
      ]),
    ]);
...
function isGroupAdmin(group, userId) {
  ...
  return (
    String(group.createdBy) === id ||
    (group.admins || []).some((adminId) => String(adminId) === id)
  );
}
  ...
  try {
    ...
    if (!canJoinOrCreateGroup(requester)) {
      return res.status(403).json({
        success: false,
        message:
          "Admin approval is required before you can create or join groups.",
      });
    }
    ...
    const users = await User.find({
      ...
      "_id email approvalStatus role trialMessageCount trialMessageLimit",
    );
    ...
    const ineligible = users.filter(
      (user) => user.email !== req.user.email && !canJoinOrCreateGroup(user),
    );
    ...
    await group.populate(
      "members",
      "name email isOnline lastSeen lastSeenPrivacy",
    );
...
app.get("/api/groups/:groupId", authenticateToken, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Unable to load group" });
  }
});
app.put("/api/groups/:groupId", authenticateToken, async (req, res) => {
  try {
    ...
    await group.populate("admins", "name email");
    io.to(group.roomId).emit("group_updated", {
      groupId: group._id,
      group: serializeGroupForResponse(group),
    });
    return res.json({ success: true, group: serializeGroupForResponse(group) });
    ...
    return res
      .status(500)
      .json({ success: false, message: "Unable to update group" });
  }
});
app.put(
  ...
  async (req, res) => {
    try {
      if (!req.file)
        return res
          .status(400)
          .json({ success: false, message: "Group image file is required" });
      const group = await Group.findOne({
        _id: req.params.groupId,
        members: req.user.userId,
      });
      ...
      if (!isGroupAdmin(group, req.user.userId)) {
        return res
          ...
          .json({
            success: false,
            message: "Only group admins can change the group image",
          });
      }
      ...
      return res
        ...
        .json({
          success: false,
          message: error.message || "Unable to upload group image",
        });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const memberEmails = Array.isArray(req.body.memberEmails)
        ? req.body.memberEmails.map(normalizeEmail).filter(Boolean)
        : [];
      if (memberEmails.length === 0) {
        return res
          .status(400)
          .json({ success: false, message: "memberEmails is required" });
      }
      const users = await User.find({ email: { $in: memberEmails } }).select(
        "_id email approvalStatus role trialMessageCount trialMessageLimit",
      );
      if (users.length !== memberEmails.length) {
        ...
        return res
          ...
          .json({
            success: false,
            message: `These users do not exist: ${missing.join(", ")}`,
          });
      }
      ...
      if (newIds.length === 0) {
        return res
          ...
          .json({
            success: false,
            message: "These users are already members of this group",
          });
      }
      ...
      return res
        .status(500)
        .json({ success: false, message: "Unable to add members" });
    }
  },
);
app.delete(
  ...
  try {
    ...
    if (!isGroupAdmin(group, req.user.userId)) {
      return res
        ...
        .json({
          success: false,
          message: "Only group admins can manage other admins",
        });
    }
    const targetId = req.params.userId;
    if (!group.members.some((id) => String(id) === String(targetId))) {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Unable to update group admins" });
  }
}
app.put(
  "/api/groups/:groupId/admins/:userId/promote",
  authenticateToken,
  (req, res) => setGroupAdminStatus(req, res, true),
);
app.put(
  "/api/groups/:groupId/admins/:userId/demote",
  authenticateToken,
  (req, res) => setGroupAdminStatus(req, res, false),
);
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Unable to load stats" });
    }
  },
);
app.get(
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Unable to load pending users" });
    }
  },
);
  ...
  try {
    ...
    if (user.role === "admin") {
      return res
        ...
        .json({
          success: false,
          message: "Cannot change the approval status of an admin account",
        });
    }
...
app.put(
  ...
  (req, res) =>
    setApprovalStatus(
      ...
      res,
      "approved",
      "Your account has been approved. You can now message and use all features without limits.",
    ),
);
app.put(
...
app.put(
  "/api/admin/users/:userId/revoke-approval",
  authenticateToken,
  requireAdmin,
  (req, res) =>
    setApprovalStatus(
      ...
      res,
      "pending",
      "Your approval has been revoked and your account is back on the free trial.",
    ),
);
  ...
  async (req, res) => {
    try {
      ...
      if (caption.length > 2000) {
        return res
          .status(400)
          .json({ success: false, message: "Caption is too long" });
      }
      let otherEmail = null;
      if (chatType === "personal") {
        ...
        if (parts.length !== 2 || !parts.includes(req.user.email)) {
          return res
            ...
            .json({
              success: false,
              message: "You are not allowed to upload to this personal room",
            });
        }
        ...
        if (createPersonalRoomId(firstUser, secondUser) !== roomId) {
          return res
            .status(400)
            .json({ success: false, message: "Invalid personal room ID" });
        }
        ...
        if (users !== 2) {
          return res
            ...
            .json({
              success: false,
              message: "Both users must exist before media can be shared",
            });
        }
        ...
        if (!groupId) {
          return res
            ...
            .json({
              success: false,
              message: "groupId is required for a group upload",
            });
        }
        const group = await Group.findOne({
          _id: groupId,
        ...
        if (!group) {
          return res
            ...
            .json({
              success: false,
              message: "You are not allowed to upload to this group",
            });
        }
      } else {
        return res
...
app.use((error, req, res, next) => {
  ...
  if (error instanceof multer.MulterError) {
    ...
    if (error.code === "LIMIT_FILE_COUNT") {
      return res
        .status(400)
        .json({ success: false, message: "Too many files selected" });
    }
    return res.status(400).json({ success: false, message: error.message });
  }
  if (
    error.message === "This file type is not supported" ||
    error.message === "Profile photo must be an image"
  ) {
...
io.on("connection", async (socket) => {
  console.log(
    `Authenticated socket connected: ${socket.id} | ${socket.data.user.email}`,
  );
...
nextApp
  ...
  .then(() => {
    ...
    httpServer.listen(PORT, "0.0.0.0", () => {
      console.log(
        `Server running on port ${PORT} (Next.js + API + Socket.IO, single process)`,
      );
    });
  })
  .catch((error) => {
